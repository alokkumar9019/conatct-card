...
import {
  FiMail, FiBriefcase, FiLink, FiShare2, FiDownload, FiX, FiArrowLeft, FiSave
} from "react-icons/fi";
import {
  FaLinkedin, FaGlobe, FaFacebook, FaXTwitter, FaWhatsapp
...
export default function VCardPage() {
  ...
  const [modalView, setModalView] = useState<"initial" | "emailForm" | "share">("initial");
    ...
    <main className="min-h-screen flex items-center justify-center p-4 sm:p-6 
      ...
      >
        ...
        <div className="p-6 sm:p-8 text-center bg-gradient-to-r from-sky-400 to-teal-400 text-white">
          ...
          <p className="text-sm sm:text-md mt-1 opacity-90">{contactInfo.title}</p>
        </div>
        ...
        <div className="bg-gradient-to-r from-sky-50 to-teal-50 p-4 sm:p-6 border-t">
          <div className="space-y-3 sm:space-y-4">
            <ActionButton onClick={openModal} isPrimary>
              <span className="mr-2"><FiDownload /></span> DOWNLOAD VCARD
            </ActionButton>
            <ActionButton onClick={handleShare}>
              <span className="mr-2"><FiShare2 /></span> SHARE THIS PAGE
            </ActionButton>
          </div>
        </div>
      </div>
              ...
              <div className="p-6 text-center">
                ...
                <div className="space-y-3">
                  <ModalButton onClick={() => setModalView("emailForm")}>
                    <span className="text-xl text-sky-600 mr-3"><FiMail/></span>
                    Send by Email
                  </ModalButton>
                  <a
                  ...
                  >
                    <span className="text-xl text-emerald-600 mr-3"><FiSave/></span>
              ...
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  <button
                    ...
                    className="text-gray-500 hover:text-gray-700"><FiArrowLeft size={20}/></button>
                  <h3 className="text-md font-semibold text-sky-600">Send by Email</h3>
                  <button
                    onClick={closeModal}
                    className="text-gray-500 hover:text-gray-700"><FiX size={20}/></button>
                </div>
                ...
                {statusMessage && (
                  <p className="text-center text-xs mt-3 text-gray-600">{statusMessage}</p>
                )}
              </div>
            )}
                ...
                <div className="flex items-center mt-4">
                  ...
                  <button
                    onClick={()=>{
                      ...
                      setCopyStatus("copied"); setTimeout(()=> setCopyStatus("idle"),1800)}}
...
function InfoRow({ icon, label, value, href }: { icon: React.ReactNode; label: string; value: string; href?: string }) {
  const content = (
    <div className="flex items-start gap-4">
      ...
      <div>
        <div className="text-xs text-gray-500 font-bold tracking-wide mb-1 uppercase">{label}</div>
...
function ActionButton({ onClick, children, isPrimary = false }: { onClick: () => void; children: React.ReactNode; isPrimary?: boolean }) {
  const base = "w-full flex items-center justify-center py-3 px-4 text-sm font-bold rounded-lg transition-all duration-200 uppercase tracking-wide shadow";
  const primary = "bg-sky-500 text-white hover:bg-sky-600";
  const secondary = "bg-white text-gray-600 border border-gray-300 hover:bg-gray-100";
  return <button onClick={onClick} className={`${base} ${isPrimary ? primary : secondary}`}>{children}</button>;
}

function ModalButton({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return <button onClick={onClick} className="w-full flex items-center p-3 text-left border rounded-lg hover:bg-sky-50 text-gray-800 font-medium text-sm transition">{children}</button>;
}

function ShareOption({ icon, label, href }: { icon: React.ReactNode; label: string; href: string }) {
  return <a href={href} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-3 py-2 px-2 rounded hover:bg-gray-50 transition mb-1">{icon}<span className="text-black text-sm font-medium">{label}</span></a>;
}
